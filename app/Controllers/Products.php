...
class Products extends BaseController
{
    ...
    $data['today_net_profit'] = $todayProfit - $todayExpenses;
        ...
        $data['total_doctor_receivables'] = $db->query("SELECT (SELECT COALESCE(SUM(qty * sale_price - discount), 0) FROM sales WHERE doctor_id IS NOT NULL) - (SELECT COALESCE(SUM(amount), 0) FROM doctor_payments) as net_receivable")->getRow()->net_receivable ?? 0;

        // 1. Real Chart Data (Last 7 Days Sales)
        $sevenDaysAgo = date('Y-m-d', strtotime('-6 days'));
        $chartSales = $saleModel->select("DATE(sale_date) as day, SUM(qty * sale_price - discount) as total")
