...
<div class="row g-4 mb-4">
    ...
    <div class="col-md-4">
        <div class="premium-list p-4 text-center border-0 shadow-sm" style="background: linear-gradient(135deg, #6366f1, #0ea5e9);">
            ...
            <?php 
                $totalItems = array_sum(array_column($inventory, 'qty'));
            ?>
            <h3 class="text-white fw-800 m-0 mt-1"><?= number_format($totalItems) ?> Units</h3>
            <div class="text-white opacity-60 small mt-1">Total loose items across all batches</div>
        </div>
    </div>
    <div class="col-md-4">
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            <?php if(empty($inventory)): ?>
                                <tr><td colspan="6" class="text-center py-5 text-muted">No saleable stock found. Please add stock first.</td></tr>
                            <?php else: ?>
                                <?php foreach($inventory as $item): ?>
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="fw-800 fs-5"><?= number_format($item['qty']) ?></div>
                                            <div class="text-muted small">Units Left</div>
                                        </td>
                                        <td class="text-end fw-bold">Rs. <?= number_format($item['price'], 2) ?></td>
                                        <td class="text-end px-5">
                                            <div class="fw-800 text-dark">Rs. <?= number_format($item['price'] * $item['qty'], 2) ?></div>
                                            <div class="text-muted small"><?= number_format($item['qty']) ?> x <?= number_format($item['price'], 2) ?></div>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
